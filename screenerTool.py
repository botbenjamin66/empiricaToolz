...
selectedTickers = list(dachTickerDict.items())[:400]
    ...
    if ticker in sampledTickers:
        try:
            ...
            if not df.empty:
                ...
                if (df['Low'].iloc[-1] < df['lowerBB'].shift(1).iloc[-1]) and (df['Volume'].iloc[-1] > (df['volume' + str(trail1)].shift(1).iloc[-1] * factor1)):
                    buySignals.append(stockName)
...
buySignalsData = {s: {'price': yf.download(dachTickerDict[s], start=startDate, end=endDate)} for s in buySignals}
for s in buySignalsData: buySignalsData[s]['info'] = yf.Ticker(dachTickerDict[s]).info
numRows = min(len(buySignalsData), 5)
fig = make_subplots(rows=numRows, cols=2, column_widths=[0.7, 0.3], shared_xaxes=True, vertical_spacing=0.1, subplot_titles=[s for s in list(buySignalsData.keys())[:numRows]] + [None] * numRows, specs=[[{"type": "xy", "secondary_y": True}, {"type": "table"}] for _ in range(numRows)])
for i, (stockName, data) in enumerate(list(buySignalsData.items())[:numRows]):
    df = data['price']
    ...
    fig.add_trace(go.Table(
        ...
        cells=dict(values=tableData, fill_color='lavender', align='left', font=dict(color='darkslategray', size=11))
    ), row=i+1, col=2)
